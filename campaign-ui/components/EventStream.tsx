...
export default function EventStream() {
	const { events, connected, paused, togglePause, clearEvents } =
		useEventStream();
			...
			<div className="flex-1 overflow-y-auto p-3 space-y-2 max-h-[500px]">
				{events.length === 0 ? (
					...
					[...events].reverse().map((event, i) => {
						const borderColor =
							EVENT_COLORS[event.type] || "border-l-gray-40";
						const time = new Date(
							event.timestamp,
						).toLocaleTimeString();
						return (
							...
							>
								<div className="flex items-center justify-between">
									<span className="text-xs font-medium text-gray-80">
										{event.type}
									</span>
									<span className="text-xs text-gray-60">
										{time}
