...
################################################################################
# Lambda Deployment Package Builder: communication Agent
# 
# This script automates the complete process of packaging the communication
# agent for AWS Lambda deployment. It:
# 1. Creates/uses Python 3.12 venv (AWS supported version)
# 2. Installs runtime dependencies (Strands AI, LLM, Jinja2, email-validator)
# 3. Copies source code from agents/communication, agents/shared, contracts/
# 4. Includes email templates
# 5. Creates AWS Lambda deployment ZIP
#
# Usage: ./build_lambda.sh
# Run from: {root}/lambda_deployment/communication/
################################################################################
...
# Script is at: {root}/lambda_deployment/communication/build_lambda.sh
...
if [ ! -d "$PROJECT_ROOT/agents/$AGENT_NAME/templates" ]; then
    echo "❌ ERROR: Templates directory not found: $PROJECT_ROOT/agents/$AGENT_NAME/templates"
    exit 1
fi
...
# Communication Agent - Runtime Dependencies
...
# Template rendering (for email templates)
Jinja2>=3.1.6
...
# Copy communication agent module (excluding __pycache__, including templates)
...
TEMPLATE_COUNT=$(find "$BUILD_DIR/agents/$AGENT_NAME/templates" -type f -name "*.txt" 2>/dev/null | wc -l)
...
for template in initial_outreach.txt follow_up.txt missing_document.txt clarification.txt qualified_confirmation.txt rejection.txt; do
    if [ ! -f "$BUILD_DIR/agents/$AGENT_NAME/templates/$template" ]; then
        echo "❌ ERROR: Missing template: $template"
        exit 1
    fi
done

echo "✅ Agent code and templates copied and validated"
echo ""
...
for dir in "agents/$AGENT_NAME" "agents/$AGENT_NAME/templates" agents/shared/tools agents/shared/models agents/shared/llm contracts; do
...
CRITICAL_IN_ZIP=(
    ...
    "agents/$AGENT_NAME/llm_tools.py"
    "agents/$AGENT_NAME/llm_prompts.py"
    "agents/$AGENT_NAME/templates/initial_outreach.txt"
    "agents/$AGENT_NAME/templates/follow_up.txt"
    "agents/shared/config.py"
    "agents/shared/__init__.py"
    "agents/shared/tools/dynamodb.py"
    "agents/shared/tools/email.py"
...
for dep in strands jinja2 email_validator; do
    if ! unzip -l "$DEPLOYMENT_ZIP" | grep -qi "$dep"; then
        echo "   ⚠️  Warning: $dep package may not be included"
    fi
done
...
echo ""
echo "📋 Contents:"
echo "   ✓ Communication Agent (agent, tools, models, config, llm_tools)"
echo "   ✓ Email Templates (initial_outreach, follow_up, etc.)"
...
echo "   ✓ Dependencies (strands-agents, boto3, pydantic, Jinja2, email-validator)"
echo ""
echo "🚀 Deploy Command:"
echo "   aws lambda create-function --function-name communication-agent \\"
...
echo "   # SendMessageRequested → Communication Agent"
echo "   aws events put-rule --name communication-send-message-trigger \\"
echo "     --event-bus-name recruitment-events-poc \\"
echo "     --event-pattern '{\"detail-type\": [\"SendMessageRequested\"]}'"
echo ""
echo "   # ReplyToProviderRequested → Communication Agent"
echo "   aws events put-rule --name communication-reply-trigger \\"
echo "     --event-bus-name recruitment-events-poc \\"
echo "     --event-pattern '{\"detail-type\": [\"ReplyToProviderRequested\"]}'"
echo ""
echo "════════════════════════════════════════════════════════════════════════════════"
