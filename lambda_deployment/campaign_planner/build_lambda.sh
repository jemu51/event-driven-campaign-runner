...
################################################################################
# Lambda Deployment Package Builder: campaign_planner Agent
# 
# This script automates the complete process of packaging the campaign_planner
# agent for AWS Lambda deployment. It:
# 1. Creates/uses Python 3.12 venv (AWS supported version)
# 2. Installs runtime dependencies (including Strands AI framework)
# 3. Copies source code from agents/campaign_planner, agents/shared, contracts/
# 4. Cleans up unnecessary files
# 5. Creates AWS Lambda deployment ZIP
#
# Usage: ./build_lambda.sh
# Run from: {root}/lambda_deployment/campaign_planner/
################################################################################
...
# Script is at: {root}/lambda_deployment/campaign_planner/build_lambda.sh
...
# Campaign Planner Agent - Runtime Dependencies
# Full agent dependencies including Strands AI framework
...
# Retry logic (used in agent tools)
tenacity>=9.1.3
...
# Copy campaign_planner agent module (excluding __pycache__)
...
for file in events.json state_machine.json requirements_schema.json; do
...
echo ""
echo "📋 Contents:"
echo "   ✓ Campaign Planner Agent (agent, tools, models, config)"
echo "   ✓ Shared Libraries (config, models, tools, llm, state_machine)"
echo "   ✓ Contracts (event schemas, state_machine, requirements_schema)"
echo "   ✓ Lambda Handler Wrapper (lambda_handler.py)"
echo "   ✓ Dependencies (strands-agents, boto3, pydantic, structlog, tenacity)"
echo ""
echo "🚀 Deploy Command:"
echo "   aws lambda create-function --function-name campaign-planner-agent \\"
...
echo ""
echo "📡 EventBridge Rule (NewCampaignRequested → Campaign Planner):"
echo "   aws events put-rule --name campaign-planner-trigger \\"
echo "     --event-bus-name recruitment-events-poc \\"
echo "     --event-pattern '{\"detail-type\": [\"NewCampaignRequested\"]}'"
echo ""
echo "════════════════════════════════════════════════════════════════════════════════"
