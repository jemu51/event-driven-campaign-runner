...
################################################################################
# Lambda Deployment Package Builder: send_follow_ups
# 
# This script automates the complete process of packaging the send_follow_ups
...
# 3. Copies source code from agents/shared, lambdas/, and contracts/
# 4. Cleans up unnecessary files
# 5. Creates AWS Lambda deployment ZIP
#
# Usage: ./build_lambda.sh
# Run from: {root}/lambda_deployment/send_follow_ups/
################################################################################
...
# Script is at: {root}/lambda_deployment/send_follow_ups/build_lambda.sh
# Going up 2 levels (../..) gets us to project root
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
BUILD_DIR="$SCRIPT_DIR"
LAMBDA_NAME="send_follow_ups"
PYTHON_VERSION="3.12"
...
# Send Follow-Ups Lambda - Runtime Dependencies
# Optimized: Only runtime deps needed (no Strands, Jinja2, email-validator, testing tools)
...
# Copy lambdas directory structure (only send_follow_ups and parent __init__.py)
...
for file in handler.py query_builder.py __init__.py; do
...
# ============================================================================
# STEP 7: COPY AGENTS/SHARED (MINIMAL - only what send_follow_ups needs)
# ============================================================================

echo "📋 Step 7: Copying agents/shared code (minimal)..."
...
# For send_follow_ups, we ONLY need:
# - agents/shared/__init__.py
# - agents/shared/state_machine.py
# - agents/shared/exceptions.py

mkdir -p "$BUILD_DIR/agents/shared"

# Copy only the minimal required files
cp "$PROJECT_ROOT/agents/shared/__init__.py" "$BUILD_DIR/agents/shared/" 2>/dev/null || touch "$BUILD_DIR/agents/shared/__init__.py"
cp "$PROJECT_ROOT/agents/shared/state_machine.py" "$BUILD_DIR/agents/shared/"
cp "$PROJECT_ROOT/agents/shared/exceptions.py" "$BUILD_DIR/agents/shared/"

SHARED_FILES=$(find "$BUILD_DIR/agents/shared" -type f -name "*.py" | wc -l)
echo "   ✓ Copied $SHARED_FILES critical Python files from agents/shared"
...
echo "✅ agents/shared code copied and validated (minimal dependency set)"
echo ""
...
for file in events.json state_machine.json dynamodb_schema.json; do
...
for dir in lambdas/send_follow_ups agents/shared contracts; do
...
# ============================================================================
# STEP 12: VALIDATE ZIP CONTENTS
# ============================================================================

echo "🔎 Step 12: Validating ZIP contents..."

CRITICAL_IN_ZIP=(
    "lambdas/send_follow_ups/handler.py"
    "lambdas/send_follow_ups/query_builder.py"
    "lambdas/send_follow_ups/__init__.py"
    "agents/shared/__init__.py"
    "agents/shared/state_machine.py"
    "agents/shared/exceptions.py"
    "contracts/events.json"
    "contracts/state_machine.json"
    "contracts/dynamodb_schema.json"
)
...
echo ""
echo "📋 Contents:"
echo "   ✓ Lambda Code (handler, query_builder)"
echo "   ✓ Shared Libraries (state_machine, exceptions - minimal set)"
echo "   ✓ Contracts (event schemas, state_machine, dynamodb_schema)"
echo "   ✓ Dependencies (boto3, pydantic, structlog, python-dateutil)"
echo ""
echo "🚀 Deploy Command:"
echo "   aws lambda create-function --function-name send-follow-ups \\"
echo "     --runtime python3.12 \\"
echo "     --handler lambdas.send_follow_ups.handler.lambda_handler \\"
...
echo "   RECRUITMENT_DYNAMODB_TABLE_NAME (required)"
echo "   RECRUITMENT_DYNAMODB_GSI1_NAME (required)"
echo "   RECRUITMENT_EVENTBRIDGE_BUS_NAME (required)"
echo "   AWS_REGION (required)"
echo "   RECRUITMENT_EVENTBRIDGE_SOURCE (optional, default: recruitment.lambdas.send_follow_ups)"
echo "   RECRUITMENT_MAX_FOLLOW_UPS (optional, default: 3)"
echo "   RECRUITMENT_MAX_RESULTS_PER_QUERY (optional, default: 100)"
echo ""
echo "📅 EventBridge Trigger:"
echo "   Create a Scheduled Rule with a cron expression (e.g., 'cron(0 0 * * ? *)')"
echo "   Target: This Lambda function"
echo "   Optional Detail: { \"custom_thresholds\": {...}, \"dry_run\": false }"
echo ""
echo "════════════════════════════════════════════════════════════════════════════════"
